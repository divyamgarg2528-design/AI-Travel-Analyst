...
4. Converted `Duration` (e.g. `"2h 50 min"`) into total minutes using a
   regex-based parser, and mapped `Total_Stops` into a numeric column.
5. Created 5 visualizations to explore relationships between price and
   other variables (airline, stops, route, duration).
6. Computed correlations and summarized key findings.
...
- The dataset file was actually an Excel spreadsheet despite an initial
  ...
  `pd.read_csv`.
- Some airline names appear multiple times with different capitalization
  (e.g. `"Qatar Airways"`, `"QATAR AIRWAYS"`, `"qatar airways"`), which
  are currently treated as separate airlines — a data quality issue worth
  normalizing in a future pass.

## Future Improvements
- Normalize airline name casing so duplicate airlines merge into one
  category.
